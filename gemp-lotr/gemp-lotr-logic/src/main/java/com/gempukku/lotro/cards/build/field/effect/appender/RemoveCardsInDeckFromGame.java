...
public class RemoveCardsInDeckFromGame implements EffectAppenderProducer {
    ...
    public EffectAppender createEffectAppender(JSONObject effectObject, CardGenerationEnvironment environment) throws InvalidCardDefinitionException {
        FieldUtils.validateAllowedFields(effectObject, "count", "filter", "player", "deck");
        ...
        result.addEffectAppender(
                CardResolver.resolveCardsInDeck(filter, null, valueSource, "_temp", player, deck, "Choose cards from deck to remove from game", environment));
        result.addEffectAppender(
                new DelayedAppender() {
                    @Override
