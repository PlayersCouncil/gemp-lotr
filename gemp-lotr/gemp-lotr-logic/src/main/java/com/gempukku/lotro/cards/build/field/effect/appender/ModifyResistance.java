...
public class ModifyResistance implements EffectAppenderProducer {
    ...
    public EffectAppender createEffectAppender(JSONObject effectObject, CardGenerationEnvironment environment) throws InvalidCardDefinitionException {
        ...
        if (limit > -1 && effectObject.get("until") != null)
            throw new InvalidCardDefinitionException("ModifyResistance: If limitPerCardThisPhase is defined, the until should not be, as it can only be applied until current phase");

        MultiEffectAppender result = new MultiEffectAppender();

        result.addEffectAppender(
                CardResolver.resolveCards(select, valueSource, memory, "you", "Choose cards to modify resistance of", environment));
        result.addEffectAppender(
                new DelayedAppender() {
                    @Override
                    protected Effect createEffect(boolean cost, CostToEffectAction action, ActionContext actionContext) {
                        ...
                        if (limit != -1) {
                            ...
                            for (PhysicalCard affectedCard : cardsFromMemory) {
                                ...
                                if (incrementedBy > 0) {
                                    modifiers.add(new ResistanceModifier(actionContext.getSource(), affectedCard, null, multiplier * incrementedBy));
                                }
                            }
                            return new AddUntilModifierEffect(modifiers, time);
                        } else {
                            return new AddUntilModifierEffect(
                                    new ResistanceModifier(actionContext.getSource(), Filters.in(cardsFromMemory), null, amount), time);
                        }
                    }
                });
