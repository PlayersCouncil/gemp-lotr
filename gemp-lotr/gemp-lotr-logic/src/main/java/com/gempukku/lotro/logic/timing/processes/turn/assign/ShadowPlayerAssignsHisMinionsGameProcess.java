...
public class ShadowPlayerAssignsHisMinionsGameProcess implements GameProcess {
    ...
    public void process(final LotroGame game) {
        ...
        final Collection<PhysicalCard> minions = Filters.filterActive(game, minionFilter, Filters.assignableToSkirmish(Side.SHADOW, false, true, false));
        if (minions.size() > 0) {
            final Collection<PhysicalCard> freePeopleTargets =
                    Filters.filterActive(game,
                            Filters.and(
                                    ...
                                    Filters.assignableToSkirmish(Side.SHADOW, false, true, false)));
