...
public class FotrPowerMaxMixedTableDraftBoosterProducer implements BoosterProducer {
    ...
    public FotrPowerMaxMixedTableDraftBoosterProducer(CollectionsManager collectionsManager, LotroCardBlueprintLibrary cardLibrary,
        ...
        // Remove MORE bad cards from pools to make boosters better
        cardPools.forEach((key, value) -> {
            if (key.startsWith("common") || key.startsWith("uncommon")) {
                value.removeIf(cardId -> !cardPlayRates.containsKey(cardId) || cardPlayRates.get(cardId) < 0.05); // (Un)common has low play rate
            }
            if (key.startsWith("rare")) {
                value.removeIf(cardId -> !cardPlayRates.containsKey(cardId) || cardPlayRates.get(cardId) < 0.02); // Rare has really low play rate
            }
        });

        // Add the rare ring to draft manually - it is neither fp nor shadow card
        cardPools.get("rare-fp").add("1_1");
        // Remove starting Frodo and common Balrog from pool
        cardPools.get("common-fp").remove("1_290");
        cardPools.get("common-shadow").remove("2_51");
    }
    ...
    public Booster getBooster(int round) {
        ...
        // Pick cards
        List<String> pickedCards = pickRandom(rareCards, 2);
        pickedCards.addAll(pickRandom(uncommonCards, 4));
        pickedCards.addAll(pickRandom(commonCards, 8));
